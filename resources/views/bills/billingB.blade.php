<h1 class="text-center text-light m-3">Kachha Order Bill</h1>
<table class="text-white table table-bordered table-primary text-center">
    ...
    <tr>
        ...
        <td>
            <a href="" data-bs-toggle="modal" data-bs-target="#exampleModal2" onclick="populateEditBForm({{ $pakka }})">
                <input class="btn btn-warning" value="Edit" type="button">
            </a>
            <a href="{{route('deleteStoreB',['id'=>$pakka->id])}}" onclick="return confirm('Are you sure?')">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- Order form -->
                <form method="post" action="" id="editorderB">
                    @csrf
                    <div class="row mb-3">
                        <label for="customerB" class="col-sm-2 col-form-label">Customer Name</label>
                        <div class="col-sm-10">
                            <input type="name" class="form-control" name="custname" id="customerB" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="itemB" class="col-sm-2 col-form-label">Item</label>
                        <div class="col-sm-10">
                            <input type="name" class="form-control" name="item" id="itemB" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="hsnB" class="col-sm-2 col-form-label">Hsn Number</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="hsn" id="hsnB" readonly>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="qtyB" class="col-sm-2 col-form-label">Quantity</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="qty" id="qtyB" onchange="calculateTotalB()" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="priceB" class="col-sm-2 col-form-label">Sell Price</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="price" id="priceB" onchange="calculateTotalB()" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="totalpriceB" class="col-sm-2 col-form-label">Total Amount</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="totalprice" id="totalpriceB" readonly>
                        </div>
                    </div>
...
<script>
    ...
    function populateEditBForm(pakka) {
        document.querySelector('#editorderB').action = `/billingB-update/${pakka.id}`;
        document.querySelector('#customerB').value = pakka.custname;
        document.querySelector('#itemB').value = pakka.item;
        document.querySelector('#hsnB').value = pakka.hsn;
        document.querySelector('#qtyB').value = pakka.qty;
        document.querySelector('#priceB').value = pakka.price;
        document.querySelector('#totalpriceB').value = pakka.totalprice;
    }
